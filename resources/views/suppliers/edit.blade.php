<x-layouts.app :headerTitle="'Edit Supplier: ' . $supplier->company_name" :headerSubtitle="'Update supplier contact details and terms'">
    <div class="p-6 max-w-2xl mx-auto">
        <div class="bg-white rounded-xl border border-slate-200/80 p-6 shadow-xs">
            <div class="flex items-center justify-between border-b border-slate-100 pb-4 mb-5">
                <h3 class="text-sm font-bold text-slate-900">Edit Supplier: {{ $supplier->company_name }}</h3>
            ...
            <form action="{{ route('suppliers.update', $supplier) }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="company_name" value="{{ old('company_name', $supplier->company_name) }}" required class="w-full text-xs rounded-lg border-slate-300 py-2">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Contact Person <span class="text-rose-500">*</span></label>
                        <input type="text" name="name" value="{{ old('name', $supplier->name) }}" required class="w-full text-xs rounded-lg border-slate-300 py-2">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Phone Number <span class="text-rose-500">*</span></label>
                        <input type="text" name="phone" value="{{ old('phone', $supplier->phone) }}" required class="w-full text-xs rounded-lg border-slate-300 py-2">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">WhatsApp Number</label>
                        <input type="text" name="whatsapp_number" value="{{ old('whatsapp_number', $supplier->whatsapp_number) }}" class="w-full text-xs rounded-lg border-slate-300 py-2">
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Email</label>
                        <input type="email" name="email" value="{{ old('email', $supplier->email) }}" class="w-full text-xs rounded-lg border-slate-300 py-2">
                    </div>
                    <div>
                        ...
                        <select name="payment_terms" class="w-full text-xs rounded-lg border-slate-300 py-2">
                            <option value="Net 30" {{ $supplier->payment_terms === 'Net 30' ? 'selected' : '' }}>Net 30 Days</option>
                            <option value="Net 15" {{ $supplier->payment_terms === 'Net 15' ? 'selected' : '' }}>Net 15 Days</option>
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="address" rows="2" class="w-full text-xs rounded-lg border-slate-300 p-2">{{ old('address', $supplier->address) }}</textarea>
                    </div>
                </div>
